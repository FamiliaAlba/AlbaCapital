  ...
  return (
    ...
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-minimal text-muted-foreground">{opportunity.tipo}</p>
          <h3 className="mt-2 text-xl font-light text-architectural">{opportunity.nombre}</h3>
          <p className="mt-1 text-sm text-muted-foreground">{opportunity.ubicacion}</p>
        </div>
        <Badge className={cn("shrink-0 rounded-full text-[10px] uppercase tracking-wider", statusStyles[opportunity.status])}>
      ...
      <div className="mt-6 flex items-center justify-between">
        <span className="text-sm text-muted-foreground">Objetivo {opportunity.capitalObjetivo}</span>
