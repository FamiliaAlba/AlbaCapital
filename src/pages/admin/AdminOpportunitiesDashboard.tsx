...
const AdminOpportunitiesDashboard = () => {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from("opportunities")
      .select(
        "id, slug, status, deal_status, display_order, updated_at, photo_path, opportunity_translations(locale, title)",
      )
      .order("display_order", { ascending: true });

    if (error) {
      toast({ title: "Error al cargar oportunidades", description: error.message, variant: "destructive" });
      setLoading(false);
      return;
    }
    ...
    setRows(
      (data ?? []).map((r) => ({
        ...
        title: r.opportunity_translations?.find((t) => t.locale === "es-AR")?.title ?? null,
      })),
    );
    setLoading(false);
  }, [toast]);
  ...
  const toggleStatus = async (row: Row, newStatus: "published" | "draft") => {
    setBusyId(row.id);
    const { error } = await supabase.from("opportunities").update({ status: newStatus }).eq("id", row.id);
    setBusyId(null);
    if (error) {
      toast({ title: "No se pudo actualizar el estado", description: error.message, variant: "destructive" });
      return;
    }
    toast({ title: newStatus === "published" ? "Oportunidad publicada" : "Oportunidad despublicada" });
    load();
  };
  ...
  const archive = async (row: Row) => {
    ...
    toast({ title: "Oportunidad archivada" });
    load();
  };
  ...
  const confirmDelete = async () => {
    ...
    if (row.photo_path) {
      await supabase.storage.from("opportunity-photos").remove([row.photo_path]);
    }
    ...
    toast({ title: `${row.title ?? row.slug} fue eliminada.` });
    load();
  };
  ...
  return (
    <AdminLayout>
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-light text-architectural">Oportunidades de inversión</h1>
          <p className="text-sm text-muted-foreground">Proyectos y activos publicados en el sitio</p>
        </div>
        <Button asChild>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-border bg-background p-4">
          <p className="text-xs text-muted-foreground">Ocultas / borrador</p>
      ...
      {loading ? (
        ...
        <p className="py-16 text-center text-sm text-muted-foreground">No hay oportunidades que coincidan.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/50 text-left text-xs text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3 hidden md:table-cell">Estado del deal</th>
            ...
            <tbody>
              {filtered.map((row, i) => (
                <tr key={row.id} className="border-b border-border last:border-0">
                  ...
                  <td className="px-4 py-3 font-medium">{row.title ?? <span className="text-muted-foreground">Sin título (ES)</span>}</td>
                  <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{DEAL_STATUS_LABEL[row.deal_status]}</td>
                  <td className="px-4 py-3">
                    <span
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="sm" asChild>
                        <Link to={`/admin/opportunities/${row.id}`} aria-label="Editar">
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(open) => !open && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>¿Eliminar "{pendingDelete?.title ?? pendingDelete?.slug}"?</AlertDialogTitle>
            <AlertDialogDescription>
              Esta acción borra la oportunidad y su fotografía de forma permanente. No se puede
              deshacer. Si preferís conservar el historial, usá "Archivar" en su lugar.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
